...
class SuffixTree:
    ...
    def load(self, data):
        ...
        self.next_weight = data["next_weight"]
    ...
    def to_dict(self):
        ...
        data["next_weight"] = self.next_weight
        data["children"] = {}
    ...
    def next_proba(self, node):
        return self.next_weight[node]/self.weight if node in self.next_weight else 0
    ...
    def get_node_info(self, lst_nodes, copy_node_lst=True):
        if copy_node_lst:
            lst_nodes = [node for node in lst_nodes]
        if lst_nodes:
            node = lst_nodes.pop()
            if node in self.children:
                return self.children[node].get_tree(lst_nodes, node, copy_node_lst=False)
        return self.weight, self.next_weight
    
    def add_all_prefix_children(self, lst_nodes, next_node, copy_node_lst=True):
        self.weight += 1
        if copy_node_lst:
            lst_nodes = [node for node in lst_nodes]
        if next_node in self.next_weight:
            self.next_weight[next_node] += 1
        else :
            self.next_weight[next_node] = 1
        if lst_nodes:
            node = lst_nodes.pop()
    ...
    def similarity_pruning(self, K=0.04):
        ...
        for node, child in self.children.items():
            ...
            non_similarity = sum([child.next_proba(node)*np.log(child.next_proba(node)/self.next_proba(node)) for node in child.next_weight.keys()])*child.weight
            if non_similarity < K:
                nodes_to_remove.append(node)
        for node in nodes_to_remove:
            self.children.pop(node)
    ...
    def stringify(self, depth=0):
        ...
        for node, weight in sorted(self.next_weight.items(), key=lambda x:-x[1]):
    ...
    def compute_entropy(self):
        self.entropy = -sum([weight*np.log(weight) for weight in self.next_weight.values()])/self.weight + np.log(self.weight)
        for child in self.children.values():
    ...
    def random_next_node(self, lst_nodes, alpha=1.,copy_node_lst=True):
        ...
        return random.choices(list(self.next_weight.keys()), [elt**alpha for elt in self.next_weight.values()])[0]
...
if __name__ == "__main__":
    ...
    output_file = "suffix_tree.json"
    net = MultiLayerNetwork(use_gui=False, enharmony=False, pitch=False, structure="monolayer", diatonic_interval=False, duration=True, midi_files=midi_files)
    tree = SuffixTree()
    ...
    node_lst = net._get_nodes_list(1)
    acc = 0
    for i in range(len(node_lst)):
        proba = tree.proba(node_lst[max(0,L-i):i],node_lst[i],False)
        acc += np.log(proba)
        # print(node_lst[i],":",round(proba,3))
    print(acc/len(node_lst))